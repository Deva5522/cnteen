...
import { LogOut, ClipboardList, Utensils, Plus, Trash2, CheckCircle, Wallet, TrendingUp, TrendingDown, History, Edit, BarChart2, PieChart, Camera, X } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const AdminDashboard = () => {
    const { logout } = useAuth();
    const {
        items, orders, transactions, mockUsers, announcements,
        addItem, removeItem, toggleItemAvailability, updateItem, updateOrderStatus, togglePaymentStatus, settleOrder, cancelOrder,
        addAnnouncement, removeAnnouncement, examMode, toggleExamMode
    } = useApp();
    const [activeTab, setActiveTab] = useState('orders'); // orders, history, menu, finances, notifications, analytics
    const navigate = useNavigate();

    // Form State
    const [newItem, setNewItem] = useState({ name: '', price: '', category: 'Snacks', description: '', image: '', stock: '50' });
    ...
    const [scanResult, setScanResult] = useState(null); // { user: 'John Doe', orders: [] }
    ...
    const startScanning = async () => {
        ...
        try {
            ...
            // Mock Match after 3s
            setTimeout(() => {
                const activeOrders = orders.filter(o => o.status === 'Ready' || o.status === 'Cooking');
                let foundOrders = [];
                let matchedUser = 'Guest';

                if (activeOrders.length > 0) {
                    // Pick random order
                    const randomOrder = activeOrders[Math.floor(Math.random() * activeOrders.length)];
                    matchedUser = randomOrder.userId;
                    foundOrders = activeOrders.filter(o => o.userId === matchedUser);
                } else {
                    matchedUser = 'Unknown User';
                }
                ...
                setIsScanning(false);
                // Stop camera after scan? Keep it open to show "Match"
            }, 3000);
    ...
    const handleAddItem = (e) => {
        ...
        if (newItem.name && newItem.price) {
            ...
            setNewItem({ name: '', price: '', category: 'Snacks', description: '', image: '', stock: '50' });
            alert('Item Added!');
        }
    };
            ...
            <main style={{ flex: 1, padding: '2rem', overflowY: 'auto' }}>
                ...
                {activeTab === 'orders' && (
                    <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                            ...
                            <button onClick={startScanning} className="btn-primary" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                <Camera size={20} /> Face Verify Pickup 🤳
                            </button>
                        </div>
                                            ...
                                            <div>
                                                ...
                                                {order.rating && (
                                                    <div style={{ marginTop: '0.5rem', fontSize: '0.85rem', color: 'gold' }}>
                                                        ...
                                                        {order.review && <span style={{ color: 'var(--text-muted)', marginLeft: '0.5rem' }}>"{order.review}"</span>}
                                                    </div>
                                                )}
                                            </div>
                ...
                {activeTab === 'finances' && (
                    <div style={{ animation: 'fadeIn 0.5s' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                            {/* USER BALANCES */}
                            <div className="card">
                                <h3 style={{ marginBottom: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>User Balances</h3>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    {mockUsers.map(u => (
                                        <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem', borderRadius: '0.5rem', backgroundColor: 'var(--bg-dark)' }}>
                                            <div>
                                                <div style={{ fontWeight: '600' }}>{u.name}</div>
                                                <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>@{u.id}</div>
                                            </div>
                                            <div style={{ fontWeight: 'bold', color: u.wallet >= 0 ? 'var(--success)' : 'var(--danger)' }}>
                                                {u.wallet >= 0 ? `₹${u.wallet}` : `-₹${Math.abs(u.wallet)}`}
                                            </div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                    ...
                                    <tbody>
                                        {items.slice(0, 5).map(item => {
                                            // Mock demand logic
                                            const trend = Math.random() > 0.5 ? 'Up' : 'Stable';
                ...
                {showScanner && (
                    <div style={{
                        ...
                        <div className="card" style={{ width: '500px', maxWidth: '90%', position: 'relative', animation: 'zoomIn 0.3s' }}>
                            <button onClick={stopScanning} style={{ position: 'absolute', top: 10, right: 10, background: 'transparent', color: 'white' }}>
                                <X size={24} />
                            </button>

                            <h3 style={{ textAlign: 'center', marginBottom: '1rem' }}>
                                {isScanning ? 'Scanning Face...' : 'Identity Verified ✅'}
                            </h3>

                            <div style={{ position: 'relative', borderRadius: '1rem', overflow: 'hidden', aspectRatio: '4/3', backgroundColor: 'black', marginBottom: '1rem' }}>
                                {isScanning && (
                                    <div style={{
                                        ...
                                        <div style={{
                                            width: '100%', height: '2px', backgroundColor: 'var(--primary)',
                                ...
                                <video
                                    ...
                                    style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: isScanning ? 1 : 0.5 }}
                                />
                            </div>
                            ...
                            {!isScanning && scanResult && (
                                <div style={{ textAlign: 'center' }}>
                                    <div style={{ fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>
                                        User: <span style={{ color: 'var(--primary)' }}>{scanResult.user}</span>
                                    </div>

                                    {scanResult.orders.length > 0 ? (
                                        <div style={{ textAlign: 'left', marginTop: '1rem', backgroundColor: 'var(--bg-dark)', padding: '1rem', borderRadius: '0.5rem' }}>
                                            <h4 style={{ marginBottom: '0.5rem' }}>Ready for Pickup:</h4>
                                            {scanResult.orders.map(o => (
                                                <div key={o.id} style={{ borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem', marginBottom: '0.5rem' }}>
                                                    <div style={{ fontWeight: 'bold' }}>Order #{o.id}</div>
                                                    <div style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>{o.items.length} items • ₹{o.total}</div>
                                                    <button
                                                        className="btn btn-primary"
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            stopScanning();
                                                            alert(`Order #${o.id} marked as PICKED UP!`);
                                                        }}
                                                    >
                                                        Mark Picked Up ✅
                                                    </button>
                                        ...
                                        <div className="text-muted" style={{ margin: '1rem 0' }}>No active orders found for this user.</div>
                                    )}

                                    <button onClick={startScanning} className="btn-outline" style={{ marginTop: '1rem' }}>scan again</button>
                                </div>
                            )}
                        </div>
                    </div>
